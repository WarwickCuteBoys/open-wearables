...
"og:url": "https://docs.openwearables.io/dev-guides/data-migrations"
"og:type": "article"
---
...
**Exception: `repair_google_energy.py` is read-only by default even without
`--dry-run`.** It requires an explicit user/date/timezone/database scope and
`--apply --before-state <new-file>` to write. It is never invoked on startup.
Read its module docstring before use; native provider re-fetch and downstream
energy invalidation are separate approved operations.
